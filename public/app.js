...
function renderAnalytics() {
  ...
  const bars = (val, max, alt) => days.map((d) => {
    ...
    return `<div class="bar-wrap"><span class="bar-val">${val(d) >= 1000000 ? (val(d) / 1000000).toFixed(2) + "M" : val(d)}</span><div class="bar ${alt ? "alt" : ""}" style="height:${h}px"></div><span class="bar-lbl">${d.slice(5)}</span></div>`;
